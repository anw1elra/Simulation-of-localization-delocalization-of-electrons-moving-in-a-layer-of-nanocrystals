...
pub fn plot_data(nqd: usize, results_dir: &PathBuf) -> std::io::Result<()> {

    //make directory
    let plot_dir = &(results_dir.join(&format!("NQD={}/plots", nqd)).into_os_string().into_string().unwrap());
    let res_dir = &(results_dir.join(&format!("NQD={}/results", nqd)).into_os_string().into_string().unwrap());
    fs::create_dir_all(plot_dir).expect("Error creating directory");
    println!("{}", res_dir);
    println!("{}", plot_dir);
    let data = crate::file::read_from_file(res_dir,"IV_curve.dat");
    let x = data[0].clone();
    let y1 = data[1].clone();
    let y2 = data[2].clone();

    let file_path: PathBuf = [plot_dir, "Velocity vs field.png"].iter().collect();
    line_and_scatter_plot(x.clone(), y1, file_path, "Velocity, nm/fs", " <Vx>  ", NamedColor::DarkBlue);
    let file_path: PathBuf = [plot_dir, "Energy vs field.png"].iter().collect();
    ...
    for j_field in 1..=crate::IV_POINTS {
        ...
        let data = crate::file::read_from_file(res_dir,fl_name);

        let y1 = data[0].clone();
        let flnm = &format!("Velocity_distribution_E-{}", j_field);
        let file_path: PathBuf = [plot_dir, flnm].iter().collect();
